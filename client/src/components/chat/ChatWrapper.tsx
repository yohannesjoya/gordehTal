...
const ChatWrapper = ({
  ...
  const handleSummary = async ()=>{
      ...
      setload(true)
      try {
        const response = await axios.post("http://localhost:8000/pdfsummary",{"url":fileUrl, "model":model})
  ...
  if (isLoading || load)
    return (
      <div className='relative min-h-full bg-zinc-50 flex divide-y divide-zinc-200 flex-col justify-between gap-2'>
        ...
        <Button className='w-full h-12' disabled>Summary</Button>
  ...
  return (
    <div className='relative h-full bg-zinc-50 flex divide-y divide-zinc-200 flex-col justify-between gap-2'>
        ...
        <Button className='w-full h-12' onClick={handleSummary}>Summary</Button>
